...
<head>
    ...
    <title>Admission Login - UCA Nexus</title>
    @fonts
...
<body class="font-sans antialiased bg-gradient-to-br from-primary-900 via-primary-800 to-sidebar min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-white tracking-tight">Admission Portal</h1>
            <p class="text-primary-200/60 mt-1 text-sm">Applicant processing and evaluation</p>
        </div>

        <div class="bg-white rounded-2xl shadow-2xl p-8">
            <h2 class="text-xl font-semibold text-slate-800 mb-1">Welcome back</h2>
            <p class="text-sm text-slate-500 mb-6">Sign in with your admission credentials.</p>

            <form method="POST" action="{{ route('login', 'admission') }}">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email') }}"
                           ...
                           placeholder="admission@example.com" required autofocus>
                    @error('email')
